...
const Create = (props) => {
  ...
  const buildPost = ({ id, content }) => {
    return { id, content, likes: [], nLikes: 0, postCategory: post.type.includes('image') ? 1 : 2, author: { id: user.id, fullname: user.fullname, avatar: user.avatar,comment:[],nComment:0,caption: 'Train Ride to Hogwarts.' } }
  }
